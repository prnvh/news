...
Set the Vercel project root directory to `code`.
...
Use the exact names above (not `NEXT_PUBLIC_*`). Redeploy **after** saving variables so the serverless function picks them up at runtime.

If you still see “not configured yet”, the previous build inlined missing env vars at build time — redeploy after pulling the latest fix that uses `getSecret()` / runtime env.
